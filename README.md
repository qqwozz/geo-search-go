...
### Первый коммит (инициализация проекта):

feat: initial commit — Geo Search API with PostGIS

- Add PostgreSQL + PostGIS database schema
- Create properties table with GEOGRAPHY(POINT, 4326)
- Add GiST index for fast geospatial search
- Implement search by radius using ST_DWithin
- Add Go API with Gin framework
- Add database health check utility
- Include test data (3 properties in Moscow)
- Add project structure with migrations
...
feat: Geo Search API with PostGIS and Go

Initial implementation of REST API for finding nearby properties
using PostgreSQL, PostGIS and Golang.

```

### Для последующих коммитов (примеры):

```bash
# Добавление фильтров
feat: add price and rooms filters to search endpoint

# Исправление ошибки
fix: correct coordinate order in ST_MakePoint (lon, lat)

# Обновление документации
docs: update README with API examples and installation guide

# Добавление тестов
test: add unit tests for distance calculation

# Оптимизация
perf: add composite index for price and rooms columns
```
...
# Создать коммит
git commit -m "feat: initial commit — Geo Search API with PostGIS"

# Добавить удалённый репозиторий (если нужно)
git remote add origin https://github.com/yourusername/geo-search-go.git
...
Поздравляю с завершением проекта! 🎉 Вы создали полноценный гео-API на Go + PostGIS.
